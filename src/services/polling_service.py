...
class PollingService:
    ...
    def __init__(self, oracle_repo: OracleRepository, mariadb_repo: MariaDbRepository,
    ...
    def poll_and_publish(self, program_name: str, max_records: int = 10000,
                        batch_size: int = 100, max_retries: int = 3) -> int:
        """
        Poll new events and publish to NATS
        
        Complete workflow:
        1. Get last successful time from tracking
        2. Fetch new events from Oracle
        3. Publish events to NATS in batches with retry
        ...
        try:
            ...
            # Step 3: Publish to NATS with retry
            published_count = self.txlog_publisher.publish_batch(events, batch_size, max_retries)
    ...
    def fetch_events_by_criteria(self, event_type: Optional[str] = None, 
